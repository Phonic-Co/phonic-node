...
try {
    await client.postAgentsNameOrIdAddCustomPhoneNumber(...);
} catch (err) {
...
```typescript
const response = await client.postAgentsNameOrIdAddCustomPhoneNumber(..., {
    headers: {
...
```typescript
const { data, rawResponse } = await client.postAgentsNameOrIdAddCustomPhoneNumber(...).withRawResponse();
